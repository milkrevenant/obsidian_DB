...
2. 본격적으로 시작하기에 앞서, 이러한 내용은 딱히 외부의 도움을 얻기가 어려운 분야라 어쩔 수 없이 출처표기가 많지 않고, 거의 대부분 직접 했음을 알린다. 그리고 글을 빠르게 읽어 나가게 하기 위해서 기본적인 용어 설명은 최소화 할 것이다.
...
5. 파라미터에 대한 이해 : 역시도 공식과 유사한 것인데, 약간의 차이는 있다. 
6. LLM(Large language Model - 대규모 언어 모델) : 언어모델이라는 것은 좀 쉽게 접근한다면 '언어뭉치' 또는 '말 뭉치' 같은 표현이 이를 적절하게 설명한다고 본다. 파라미터와 프롬프트가 언어를 찾아주는 지름길, 설명서, 메뉴얼 같은 것이라면, 언어모델은 일종의 '지도' 자체와 유사하다. 언어모델이 클 수록 더 많은 데이터들이 들어가 있기 때문에 일반적으로 우리가 알고 있는 Chatgpt는 gpt3.5, gpt4라는 대규모 언어 모델을 기반으로 하고 있다.
7. 어쨌거나 이러한 자료들을 이용해서 내가 기본적으로 요구하게 된 프롬프트의 내용은 다음과 같다.
 >>> <프롬프트 시작>
...
6. 선택지의 예시(example)을 넣지 못한 이유는, 선택지야 말로 본문을 인용해서 만들어야 만 하는 복잡한 놈이기 때문에, 최대한 GPT의 랜덤성에 기대하여 더 좋은 결과가 나오기 만을 기다리는 게 쉬웠기 때문이다. 물론 내가 원하는 형태의 선택지들을 넣을 수는 있으나 '토큰'이 부족할 걱정을 해야하고, 
7. '주제통합적 독서'문항들은 대체로 수능 독서 문항들 중 2지문이 엮여 있는 문항들에서 출제되는 문항인데 시간도 오래걸리는 편이고, 선택지도 짧지 않아서 해석이 오래걸리는 글들이다. 예를 들면 이런 문항들이 있다. 역시 이런 문항들을 먼저 '텍스트'들로 바꿔주는 작업을 하고, 그 문제들에 대해 학습을 시키고, 이런 형태의 문제들을 만들어달라고 요구를 하게 되면, 그런 요구를 최대한 반영시켜주는 결과들을 불러와준다. 
8. 
9. 
10. 여기까지 따라왔다면, 독서는 이 정도면 아주 기초적인 문제들을 만드는 수준까지는 왔다. 하지만 사실 더 중요한 건 글을 이해하는 과정에서의 설명자료가 필요하다. 그걸 도출해보는 프롬프트로 넘어가자.
11. 
